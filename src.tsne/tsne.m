function ydata = tsne(X, labels, no_dims, initial_dims, perplexity, verbosity)
...
if ~initial_solution
   dispv('Preprocessing data using PCA...', verbosity);
...
P = d2p(D, perplexity, 1e-5, verbosity);        % compute affinities using fixed perplexity
clear D

% Run t-SNE
if initial_solution
   ydata = tsne_p(P, labels, ydata, verbosity);
else
   ydata = tsne_p(P, labels, no_dims, verbosity);
end
